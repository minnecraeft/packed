...
for VARIANT in "${VARIANTS[@]}"; do
  ...
  for TIER in "${TIERS[@]}"; do
    ...
    envsubst <"${BASE}/presets/barrel/recipe_${TIER}.json" >"${RECIPES}/${VARIANT}/${file}.json"

    [[ ! -d "${RECIPE_ADVANCEMENTS}/${VARIANT}" ]] && mkdir -p "${RECIPE_ADVANCEMENTS}/${VARIANT}"
    envsubst < "${BASE}/presets/barrel/advancement_${TIER}.json" > "${RECIPE_ADVANCEMENTS}/${VARIANT}/${file}.json"
